...
def main():
    ...
    while seg_date < last:
        ...
        range_df = query_date(connection, mhw_tbl,
                              seg_date.strftime("%b %d %Y"))
        ...
        for lat in np.arange(MIN_LAT, MAX_LAT, width):
            for lon in np.arange(MIN_LON, MAX_LON, width):
                ...
                masked = sst_ref[lat_to_row(lat_start, true_min_lat, frame_size):lat_to_row(lat_end, true_min_lat, frame_size), lon_to_col(
                    lon_start, true_min_lon, frame_size):lon_to_col(lon_end, true_min_lon, frame_size)].count()
                ...
                if masked > 0:
                    ...
                    seg_map = np.zeros(
                        (int(width/frame_size), int(width/frame_size)))
                    ...
                    lats, lons = get_lats_lons_from_day(
                        range_df, lat_start, lon_start, width, 0, 1)
                    ...
                    lats, lons = convert_ll_to_rc(np.array(lats), np.array(
                        lons), lat_start, lon_start, frame_size)
